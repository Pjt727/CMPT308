...
A primary key is the candidate key which is chosen to uniquely identify the row.
It is also very important to consider the business rules associated with the database when selecting keys.
A primary key should be functionally unique within its domain.
\section{Data Types in Tables}
Without data types whatever memory stored is uselesss series of 
   binary digits. 
Giving your datatypes the most strict type that possible
   is crucial to getting the most information out of your data.
...
It may be better to not even have grade-level stored and instead
   have it be derived from credits earned or courses passed.
There is much more information you could include such as
   gender, race, address, previous schooling etc.
Each of these bits of information needs to be meticously mapped
   to accurate data types when translating them to a column or columns.
...
The first normal form maintains that there are no multi-valued structures within a 
    field or fields which then relate to structure within the business which
    semantically should be separated in structure.
...
As the database's internal structure is hidden from the user or programmers,
   accessing data using concepts such as index does not make sense.
This makes sense because in postgres, for example, all tables and columns are 
   themselves stored in a database.
This means there is no concept of index as the underlying structure likely used is a set.

\subsection{Unique Rows}
Every row within a table should have a unique primary key rules of 
   which have been previously mentioned.
This is need to differinate and assert no duplication of similar or the same 
