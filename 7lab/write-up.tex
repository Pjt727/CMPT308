...
\end{tabular}
\end{table}
\subsection{(1) My professional consulting opinion}
...
Asumming this data is stored in a csv, excel, or some
    ...
    the data is difficult to programmatically add, update, delete
...
\subsection{(2) Spreadsheet in 1NF}
\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|c|}
\hline
...
\end{tabular}
\end{table}
\subsection{(5) Functional Dependencies}
The functional dependencies are sets of columns where some
    column(s) are dependent on their value(s) from other column(s).
In this table, PackageName is dependent on the PackageId and
    ComputerModel is dependent on TagNumber which makes these two
    functional dependencies.
\subsection{(6) Non 3NF-ness}
...
Both of the aforementioned functional dependencies create
...
This way you would only need to write the PackageName once
    in the spreadsheet.
\section{Part three}
\subsection{(7) Primary Keys}
The primary keys for the tables to be split is the packageId, TagNumber,
    and TagNumber + PackageID respectfully.
\subsection{(8) Functional dependencies}
The functional dependencies for each table is only its primary key: Packages is
    PackageId, Installations is TagNumber and PackageId, finally Computers is
    TagNumber.
These are the functional dependencies because the other columns only rely on those
    columns.
\subsection{(9) 3NF}
The tables are in third normal form because because there are no non-key or partial
    key functional dependencies. There are also no functional dependencies for any other
    columns onto another.
The data also follows the previous forms of having no feilds with multiple items
    following a structure in just one column.

\newpage
\subsection{(10) Lucid Chart Diagram}
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{assets/lucid_chart_db.png}
  \caption{Crow's nest ER diagram}
  \label{fig:lucid_chart_db}
\end{figure}
\end{document}
